...
내용이 길어보이지만 결국 핵심은
```
"깃(Git)"은 버전 관리 시스템(VCS, Version Control System) 중 하나로, 소스 코드와 같은 파일의 변경 사항을 추적하고 관리하는 도구입니다. 개발자들은 Git을 사용하여 프로젝트의 이전 상태로 쉽게 돌아갈 수 있으며, 여러 명의 개발자가 동시에 작업할 수 있도록 협업을 지원합니다.
```
...
github페이지로 가서 레포지토리 생성 버튼을 누르면 위와 같은 화면이 나오게 된다. 핵심적으로 봐야하는 부분들을 1번 ~ 4번으료 표시해 두었다.  
 ...
 Private : 해당 방식으로 설정하면 원격 레포지토리를 만들고나서 해당 레포지토리의 설정을 들어가 초대할 Member의 아이디(git 이메일)를 입력한다. 그러면 초대받은 Member만이 해당 원격 레포지토리에 접근해서 볼 수 있는 것이다.  
...
그리고 이후에 git에 대한 명령어와 동작방식을 어느정도 이해했을 때 SoureceTree라고 하는, git을 GUI형태로 편하게 쓰는 방법으로 넘어갈 생각이다.  
