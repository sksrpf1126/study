...
서블릿의 구조는 인터페이스로 구성이 되어 있으며, 큰 흐름은 클라이언트(브라우저)의 요청에 담겨있는 URL과 HTTP 메서드와 서블릿에 정의를 해둔 URL 경로와 HTTP 메서드(GET, POST 등)가 일치하는 서블릿 클래스가 존재한다면 일치하는 서블릿 클래스를 통해 ***단 하나의 객체*** 만을 만들어둔다. (싱글톤 패턴)  
